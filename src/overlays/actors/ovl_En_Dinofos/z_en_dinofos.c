...
void EnDinofos_Init(Actor* thisx, PlayState* play) {
    ...
    for (i = 6; i < ARRAY_COUNT(this->colliderJntSphElements); i++) {
...
void EnDinofos_Blink(EnDinofos* this) {
    ...
    } else if (Rand_ZeroOne() < DINOFOS_PROB_BLINK) {
        this->eyeTexIndex = 1;
    }
}
...
void EnDinofos_ChooseAction(EnDinofos* this, PlayState* play) {
    if (EnDinofos_IsFacingPlayer(this)) {
        if (this->actor.xzDistToPlayer < 100.0f) {
            if (
                (((this->actionFunc != EnDinofos_SlashFromGround) && (Rand_ZeroOne() > DINOFOS_PROB_COMPL_CHOOSE_ACTION_NOT_SLASHING_TO_SLASHING)) ||
                 ((this->actionFunc == EnDinofos_SlashFromGround) && (Rand_ZeroOne() > DINOFOS_PROB_COMPL_CHOOSE_ACTION_SLASHING_TO_SLASHING)))) {
            ...
            if (((this->actionFunc != EnDinofos_Land) && (Rand_ZeroOne() < DINOFOS_PROB_CHOOSE_ACTION_NOT_LANDING_TO_JUMP_FORWARD)) ||
                ((this->actionFunc == EnDinofos_Land) && (Rand_ZeroOne() < DINOFOS_PROB_CHOOSE_ACTION_LANDING_TO_JUMP_FORWARD))) {
                EnDinofos_SetupChooseJump(this, DINOFOS_JUMP_TYPE_FORWARD);
            } else {
                EnDinofos_SetupCircleAroundPlayer(this, play);
            }
        } else if (Rand_ZeroOne() < DINOFOS_PROB_CHOOSE_ACTION_TO_WALK) {
            EnDinofos_SetupWalk(this, play);
        } else {
            EnDinofos_SetupCircleAroundPlayer(this, play);
        }
    } else if (Rand_ZeroOne() < DINOFOS_PROB_CHOOSE_ACTION_TO_TURN_TO_PLAYER) {
...
void EnDinofos_Freeze(EnDinofos* this) {
    ...
    this->stunTimer = DINOFOS_FREEZE_TIMER;
...
void EnDinofos_EnableBumperCollision(EnDinofos* this) {
    ...
    if (this->isDodgingGoronPound) {
        for (i = 0; i < ARRAY_COUNT(this->colliderJntSphElements) - 3; i++) {
...
s32 EnDinofos_Dodge(EnDinofos* this, PlayState* play) {
    ...
    if ((this->actor.xzDistToPlayer < 100.0f) && (player->meleeWeaponState != PLAYER_MELEE_WEAPON_STATE_0) &&
        this->actor.isLockedOn && (Rand_ZeroOne() < DINOFOS_PROB_JUMP_FROM_PLAYER_MELEE_ATTACK) && EnDinofos_IsFacingPlayer(this) &&
        Player_IsFacingActor(&this->actor, 0x2000, play)) {
        if (Rand_ZeroOne() < DINOFOS_PROB_JUMP_BACKWARD_FROM_PLAYER_MELEE_ATTACK) {
...
void EnDinofos_SetupIdle(EnDinofos* this) {
    ...
    this->actionTimer = (s32)Rand_ZeroFloat(DINOFOS_IDLE_TIMER1_MAX_OFFSET) + DINOFOS_IDLE_TIMER1_MIN;
    this->idleTimer = DINOFOS_IDLE_TIMER;
...
void EnDinofos_SetupWalk(EnDinofos* this, PlayState* play) {
    ...
    if (this->actionFunc != EnDinofos_Walk) {
        ...
        if (this->actor.xzDistToPlayer <= targetDist) {
            this->actor.speed = -DINOFOS_WALK_SPEED_START;
        } else {
            this->actor.speed = DINOFOS_WALK_SPEED_START;
        }
    }

    this->walkTimer = (s32)Rand_ZeroFloat(DINOFOS_WALK_TIMER_MAX_OFFSET) + DINOFOS_WALK_TIMER_MIN;
    this->actor.world.rot.y = this->actor.shape.rot.y;
...
void EnDinofos_Walk(EnDinofos* this, PlayState* play) {
    ...
    if (!EnDinofos_Dodge(this, play)) {
        ...
        this->actor.world.rot.y = this->actor.shape.rot.y;
        if (this->actor.xzDistToPlayer <= targetDist) {
            Math_StepToF(&this->actor.speed, -DINOFOS_WALK_SPEED_MAX, 0.5f);
        } else {
            Math_StepToF(&this->actor.speed, DINOFOS_WALK_SPEED_MAX, 0.5f);
        }
...
void EnDinofos_SetupCircleAroundPlayer(EnDinofos* this, PlayState* play) {
    ...
    if (this->actionFunc != EnDinofos_CircleAroundPlayer) {
        ...
        if (ABS_ALT(rotY) > 0x7800) {
            if (Rand_ZeroOne() < DINOFOS_PROB_SIDESTEP_LEFT) {
                this->actor.speed = DINOFOS_CIRCLING_SPEED;
            } else {
                this->actor.speed = -DINOFOS_CIRCLING_SPEED;
            }
        } else if (rotY >= 0) {
        ...
        if (this->actionFunc == EnDinofos_DodgeProjectile) {
            this->skelAnime.playSpeed = this->actor.speed * (1.0f / DINOFOS_CIRCLING_SPEED);
        } else {
            Animation_Change(&this->skelAnime, &gDinolfosSidestepAnim, this->actor.speed * (1.0f / DINOFOS_CIRCLING_SPEED), 0.0f, 0.0f,
                             ANIMMODE_LOOP, -4.0f);
        }

        this->actor.world.rot.y = BINANG_ADD(this->actor.shape.rot.y, 0x4000);
        this->circlingRate = 0;
        this->sidestepTimer = (s32)Rand_ZeroFloat(DINOFOS_SIDESTEP_TIMER_MAX_OFFSET) + DINOFOS_SIDESTEP_TIMER_MIN;
    }
...
void EnDinofos_SetupJumpWithoutSlashing(EnDinofos* this) {
    if (this->jumpType == DINOFOS_JUMP_TYPE_BACKWARD) {
        this->actor.speed = -DINOFOS_JUMP_BACKWARD_SPEED;
        this->actor.velocity.y = DINOFOS_JUMP_BACKWARD_VELOCITY_Y;
        this->colliderJntSph.base.acFlags |= AC_ON;
    } else {
        this->actor.velocity.y = DINOFOS_JUMP_NOT_BACKWARD_VELOCITY_Y;
        if (this->jumpType == DINOFOS_JUMP_TYPE_FORWARD) {
            this->actor.speed = DINOFOS_JUMP_FORWARD_SPEED;
        }
    }
...
void EnDinofos_SetupJumpSlash(EnDinofos* this) {
    ...
    this->actor.velocity.y = DINOFOS_JUMP_SLASH_VELOCITY_Y;
    Actor_PlaySfx(&this->actor, NA_SE_EN_RIZA_JUMP);
    this->attackTimer = 0;
    this->timer2 = -1;
...
void EnDinofos_Land(EnDinofos* this, PlayState* play) {
    if (SkelAnime_Update(&this->skelAnime)) {
        if ((this->isJumpingBackward == true) && (this->actor.xzDistToPlayer < 280.0f) &&
            EnDinofos_IsFacingPlayer(this) && (Rand_ZeroOne() < DINOFOS_PROB_JUMP_BACKWARD_TO_FIRE)) {
            this->flameMultiplier = 1.5f;
            EnDinofos_SetupStartBreatheFire(this);
        } else {
            EnDinofos_ChooseAction(this, play);
        }
...
void EnDinofos_SetupSlashFromGround(EnDinofos* this) {
    ...
    this->attackTimer = 0;
    this->timer2 = -1;
    this->actor.speed = DINOFOS_SLASH_FROM_GROUND_SPEED;
    this->actor.world.rot.y = this->actor.shape.rot.y;
...
void EnDinofos_SlashFromGround(EnDinofos* this, PlayState* play) {
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        ...
        Actor_PlaySfx(&this->actor, NA_SE_EN_RIZA_ATTACK);
    } else if (this->skelAnime.curFrame < DINOFOS_SLASH_ANIM_LAST_FRAME_TO_DODGE) {
...
void EnDinofos_SetupRecoilFromBlockedSlash(EnDinofos* this) {
    ...
    if (this->actionFunc != EnDinofos_JumpSlash) {
        ...
        this->actor.speed = DINOFOS_RECOIL_SPEED;
    }
...
void EnDinofos_RecoilFromBlockedSlash(EnDinofos* this, PlayState* play) {
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        if (Rand_ZeroOne() < DINOFOS_PROB_RECOIL_TO_FIRE){
            this->flameMultiplier = 0.5f;
            EnDinofos_SetupStartBreatheFire(this);
        }
        EnDinofos_ChooseAction(this, play);
    }
...
void EnDinofos_SetupBreatheFire(EnDinofos* this, PlayState* play) {
    ...
    this->attackTimer = DINOFOS_FIRE_TIMER;
    this->actor.speed = 0.0f;
...
void EnDinofos_BreatheFire(EnDinofos* this, PlayState* play) {
    ...
    for (i = 6; i < end; i++) {
        dim = &this->colliderJntSph.elements[i].dim;
        fireRotY = 
            (s32)(Math_CosF((this->attackTimer + ((i - 5) << 1)) * (M_PIf / DINOFOS_FIRE_TIMER)) * 0x2C00) + this->actor.shape.rot.y;

        dim->worldSphere.center.x =
            (s32)this->bodyPartsPos[DINOFOS_BODYPART_JAW].x + this->flameMultiplier * (s32)(Math_SinS(fireRotY) * dim->modelSphere.center.z);
        ...
        dim->worldSphere.center.z =
            (s32)this->bodyPartsPos[DINOFOS_BODYPART_JAW].z + this->flameMultiplier * (s32)(Math_CosS(fireRotY) * dim->modelSphere.center.z);
    }
...
void EnDinofos_SetupDodgeProjectile(EnDinofos* this, s16 angleToProjectile) {
    if (angleToProjectile >= 0) {
        this->actor.speed = -DINOFOS_DODGE_PROJECTILE_SPEED;
    } else {
        this->actor.speed = DINOFOS_DODGE_PROJECTILE_SPEED;
    }
    Animation_Change(&this->skelAnime, &gDinolfosSidestepAnim, this->actor.speed * (2.0f / DINOFOS_DODGE_PROJECTILE_SPEED ), 0.0f, 0.0f,
                     ANIMMODE_LOOP, -4.0f);
    this->actor.world.rot.y = BINANG_ADD(this->actor.shape.rot.y, 0x4000);
    this->sidestepTimer = DINOFOS_DODGE_PROJECTILE_TIMER;
...
void EnDinofos_DodgeProjectile(EnDinofos* this, PlayState* play) {
    ...
    this->skelAnime.playSpeed =
        (1.0f + fabsf(this->actor.speed * (1.0f / DINOFOS_DODGE_PROJECTILE_SPEED))) * ((this->actor.speed >= 0.0f) ? 1.0f : -1.0f);
    this->actor.world.rot.y = BINANG_ADD(this->actor.shape.rot.y, 0x4000);
    SkelAnime_Update(&this->skelAnime);
    if (this->sidestepTimer != 0) {
        this->sidestepTimer--;
